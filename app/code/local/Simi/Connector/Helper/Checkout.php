...
class Simi_Connector_Helper_Checkout extends Mage_Core_Helper_Abstract {
	...
	public function getCODfee() {
		if (Mage::app()->getStore()->getCurrentCurrencyCode() == 'SAR') {
			return 25;
		}
		return 15;
	}
    ...
    public function soptName($name) {
        ...
        if (count($name) == 1)
        {
            //$lengh = strlen($name[0]) > 1 ? strlen($name[0])-1 : 1;
            //$name = str_split($name[0],$lengh);
            //$firstName = $name[0];
    ...
    public function setTotal($total, &$data) {
		...
		//hainh customize
		//var_dump(Mage::getSingleton('checkout/session')->getQuote());die;
		if ((Mage::getSingleton('checkout/type_onepage')->getQuote()->getPayment()->getMethod()) && (Mage::getSingleton('checkout/type_onepage')->getQuote()->getPayment()->getMethodInstance()->getCode() == 'cashondelivery')) {
			$data['cod_fee'] = $this->getCODfee();
		}
		//end
