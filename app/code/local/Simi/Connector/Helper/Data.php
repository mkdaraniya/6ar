 ...
 * @author      Developer
 */
class Simi_Connector_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function importDesgin() {
        ...
        foreach ($websites as $website) {
            ...
            foreach ($data as $item) {
                $model = Mage::getModel('connector/design');
                $model->setData($item);
    ...
    public function getConfigNotice($cofig, $device_id) {
        $name_device = $this->getNameDeviceById($device_id);
        $path = 'connector/notice/' . $name_device . '/' . $cofig;
        return (string) Mage::getStoreConfig($path);
    }

    public function getConfigSettings($config) {
        return (string) Mage::getStoreConfig('connector/general/' . $config);
    }
