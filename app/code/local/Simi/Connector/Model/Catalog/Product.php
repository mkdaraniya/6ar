...
class Simi_Connector_Model_Catalog_Product extends Simi_Connector_Model_Catalog {
    ...
    public function getListProduct($collection, $offset, $limit, $width, $height) {
    ...
    public function getDetail($data) {
        ...
        $_product = array(
            ...
            'product_description' => Mage::helper('catalog/output')->productAttribute($product, $product->getDescription(), 'description'),
        ...
        foreach ($attributes as $attribute) {
            if ($attribute->getIsVisibleOnFront()) {
				...
				if ($attribute->getData('attribute_code') == 'price') {
					//$priceValue = substr($attribute->getFrontend()->getValue($product),0, -5);
					$priceValue = Mage::app()->getStore()->convertPrice($product->getPrice(), false);
					if (Mage::getModel('connector/config_app')->getCurrencyPosition() == 'before')
						$priceValue = Mage::app()->getStore()->getCurrentCurrencyCode().' '.$priceValue;
					else 
						$priceValue = $priceValue.' '.Mage::app()->getStore()->getCurrentCurrencyCode();
					//if (Mage::getModel('connector/config_app')->getCurrencyPosition()
					$result[] = array(
