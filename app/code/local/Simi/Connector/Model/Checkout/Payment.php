            ...
            if ($code == "checkmo") {
				...
				if ($method->getCode() == 'cashondelivery') {
					if (!Mage::getSingleton('checkout/type_onepage')->getQuote()->getPayment()->getMethod()) {
						...
						$paymentData->payment_method = $method->getCode();
						//$this->savePaymentMethod($paymentData);
							$this->_getOnepage()->savePayment(array('method'=>$method->getCode()));
							$this->_getOnepage()->getQuote()->collectTotals()->save();
							$detail['p_method_selected'] = true;
					}
					$detail['title'] .= ' ('.Mage::app()->getStore()->getCurrentCurrencyCode().Mage::helper('connector/checkout')->getCODfee().')';
				}
				//end
                $detail['show_type'] = 0;
            }
