...
class Simi_Connector_Helper_Customer extends Mage_Core_Helper_Abstract {
    ...
    public function defaultAdress($address) {
        ...
        return array(
			...
			'po_box' => $address->getData('po_box'),
			'carriercode' => $address->getData('carriercode'),
        );
    }
    ...
    public function getAddress($data, $customer) {
        return array(
            ...
            'adddres_email' => $customer->getEmail(),
			'adddres_prefix' => $customer->getPrefix(),
			'address_po_box' => $customer->getData('po_box'),
        );
    }
    ...
    public function getAddressToOrder($data, $customer, $address_billing_id, $address_shipping_id, $id = '') {
        ...
        return array(
			...
			'prefix' => $customer->getPrefix(),
			'po_box' => $data->getData('po_box'),
			'carriercode' => $data->getData('carriercode'),
        );
    }
    ...
    public function convertDataAddress($data, $state_id) {
		if ((!$data->first_name) && (!$data->last_name))        
			...
			$name = array('first_name'=>$data->first_name,'last_name'=>$data->last_name);
		
        /*additional fields*/
        $latlng = isset($data->latlng) == true ? $data->latlng : '';
        $address = array();
        $address['prefix'] = $name['prefix'];
		...
		$address['carriercode'] = isset($data->carriercode) == true ? $data->carriercode : '';
		$address['po_box'] = isset($data->po_box) == true ? $data->po_box : '';
		if (isset($data->prefix) && Mage::getSingleton('customer/session')->isLoggedIn()) {
			try{
				$customer = Mage::getModel('customer/customer')->load(Mage::getSingleton('customer/session')->getCustomer()->getId());
		...
		else if (isset($data->prefix))
			Mage::getSingleton(‘core/session’)->setSimiPrefix($data->prefix);
        return $address;
    }
